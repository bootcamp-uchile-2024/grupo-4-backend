...
export class UpdateProductoDto {
    ...
    @ApiProperty({ description: 'Descripción del producto', example: 'Descripción del producto A' })
    @IsString()
    @IsNotEmpty()
    descripcion: string;
    ...
    @ApiProperty({ description: 'Fecha de creación del producto', example: '2023-01-01T00:00:00.000Z' })
    @IsDate()
    @IsNotEmpty()
    @Type(() => Date)
    fecha: Date;
  
    @ApiProperty({ description: 'Categorías del producto', example: ['Categoría A', 'Categoría B'] })
    @IsString({ each: true })
    @IsNotEmpty()
